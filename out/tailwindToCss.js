...
exports.COLOR_PREFIX_MAP = exports.COLOR_MAP = exports.SIZE_PREFIX_MAP = exports.SPACING_PREFIX_MAP = exports.STATIC_MAP = exports.VARIANT_MAP = void 0;
exports.tailwindClassToCss = tailwindClassToCss;
exports.tailwindClassesToCss = tailwindClassesToCss;
exports.tailwindClassesToCssDeclarations = tailwindClassesToCssDeclarations;
// =====================================================================
// VARIANT SYSTEM
// =====================================================================
exports.VARIANT_MAP = {
...
function stripVariants(cls) {
    ...
    while (true) {
        ...
        if (colonPos === -1)
            break;
        if (bracketStart !== -1 && bracketStart < colonPos)
            break;
        const candidate = remaining.slice(0, colonPos);
        if (exports.VARIANT_MAP[candidate]) {
            variantRules.push(exports.VARIANT_MAP[candidate]);
 ...
 */
function wrapWithVariants(variantRules, declarations) {
...
for (const mode of MIX_BLEND_MODES) {
    exports.STATIC_MAP[`mix-blend-${mode}`] = `mix-blend-mode: ${mode}`;
}
// =====================================================================
// SPACING MAP
// =====================================================================
exports.SPACING_PREFIX_MAP = {
    p: "padding",
 ...
 */
function resolveColorClass(cls) {
    ...
    for (const [prefix, cssProp] of Object.entries(exports.COLOR_PREFIX_MAP)) {
        const prefixDash = prefix + "-";
        if (!classWithoutOpacity.startsWith(prefixDash))
            continue;
        const rest = classWithoutOpacity.slice(prefixDash.length);
        // Special single-word colors: transparent, current, inherit, white, black
        if (exports.COLOR_MAP[rest] !== undefined) {
            const colorVal = exports.COLOR_MAP[rest];
            if (opacity) {
        ...
        if (colorShadeMatch && exports.COLOR_MAP[colorShadeMatch[1]]) {
            const colorName = colorShadeMatch[1];
            const shade = colorShadeMatch[2];
            if (opacity) {
                return `${cssProp}: ${colorName}-${shade} /* ${exports.COLOR_MAP[colorName]} shade, ${opacity}% opacity */`;
            }
            return `${cssProp}: ${colorName}-${shade} /* ${exports.COLOR_MAP[colorName]} shade */`;
        }
    }
    return null;
}
...
function resolveBaseClass(cls) {
    ...
    if (exports.STATIC_MAP[cls])
        return exports.STATIC_MAP[cls];
    ...
    if (borderWidthMatch)
        return `border-width: ${borderWidthMatch[1]}px`;
    // Ring width: ring-{n}
    ...
    for (const [prefix, cssProp] of Object.entries(exports.SIZE_PREFIX_MAP)) {
        const escaped = prefix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const sizeRegex = new RegExp(`^${escaped}-(.+)$`);
        const sizeMatch = cls.match(sizeRegex);
        if (sizeMatch)
            return `${cssProp}: ${sizeValueToCSS(sizeMatch[1])}`;
    }
    ...
    if (basisMatch)
        return `flex-basis: ${sizeValueToCSS(basisMatch[1])}`;
    ...
    if (perspectiveMatch)
        return `perspective: ${perspectiveMatch[1]}px`;
    // Order: order-{n}
    ...
    if (colStartMatch)
        return `grid-column-start: ${colStartMatch[1]}`;
    const colEndMatch = cls.match(/^col-end-(\d+|auto)$/);
    if (colEndMatch)
        return `grid-column-end: ${colEndMatch[1]}`;
    // Line clamp
    ...
    if (decoThickMatch)
        return `text-decoration-thickness: ${decoThickMatch[1]}px`;
    // Divide width
    const divideXMatch = cls.match(/^divide-x-(\d+)$/);
    if (divideXMatch)
        return `border-left-width: ${divideXMatch[1]}px /* > * + * */`;
    if (cls === "divide-x")
        return "border-left-width: 1px /* > * + * */";
    const divideYMatch = cls.match(/^divide-y-(\d+)$/);
    if (divideYMatch)
        return `border-top-width: ${divideYMatch[1]}px /* > * + * */`;
    if (cls === "divide-y")
    ...
    if (brightnessMatch)
        return `filter: brightness(${parseInt(brightnessMatch[1]) / 100})`;
    const contrastMatch = cls.match(/^contrast-(\d+)$/);
    if (contrastMatch)
        return `filter: contrast(${parseInt(contrastMatch[1]) / 100})`;
    const saturateMatch = cls.match(/^saturate-(\d+)$/);
    if (saturateMatch)
        return `filter: saturate(${parseInt(saturateMatch[1]) / 100})`;
    const hueRotateMatch = cls.match(/^hue-rotate-(\d+)$/);
    if (hueRotateMatch)
        return `filter: hue-rotate(${hueRotateMatch[1]}deg)`;
    ...
    if (backdropBlurMap[cls])
        return backdropBlurMap[cls];
    const backdropBrightnessMatch = cls.match(/^backdrop-brightness-(\d+)$/);
    if (backdropBrightnessMatch)
        return `backdrop-filter: brightness(${parseInt(backdropBrightnessMatch[1]) / 100})`;
    const backdropContrastMatch = cls.match(/^backdrop-contrast-(\d+)$/);
    if (backdropContrastMatch)
        return `backdrop-filter: contrast(${parseInt(backdropContrastMatch[1]) / 100})`;
    const backdropSaturateMatch = cls.match(/^backdrop-saturate-(\d+)$/);
    if (backdropSaturateMatch)
        return `backdrop-filter: saturate(${parseInt(backdropSaturateMatch[1]) / 100})`;
    const backdropHueRotateMatch = cls.match(/^backdrop-hue-rotate-(\d+)$/);
    if (backdropHueRotateMatch)
        return `backdrop-filter: hue-rotate(${backdropHueRotateMatch[1]}deg)`;
    const backdropOpacityMatch = cls.match(/^backdrop-opacity-(\d+)$/);
    if (backdropOpacityMatch)
        return `backdrop-filter: opacity(${parseInt(backdropOpacityMatch[1]) / 100})`;
    // Stroke width: stroke-{n}
    const strokeWidthMatch = cls.match(/^stroke-(\d+)$/);
    if (strokeWidthMatch)
        return `stroke-width: ${strokeWidthMatch[1]}`;
    return null;
}
// =====================================================================
// MAIN EXPORTS
// =====================================================================
function tailwindClassToCss(cls) {
...
function tailwindClassesToCss(classString) {
...
function tailwindClassesToCssDeclarations(classString) {
    const classes = classString.trim().split(/\s+/).filter(Boolean);
    if (classes.length === 0)
        return "";
    const lines = [];
...
//# sourceMappingURL=tailwindToCss.js.map