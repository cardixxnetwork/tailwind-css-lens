 ...
 */
async function tailwindClassToCss(cls) {
    // For variant classes, the engine handles them directly
    const result = await (0, tailwindEngine_1.resolveClass)(cls);
    return result;
}
async function tailwindClassesToCss(classString) {
    const classes = classString.trim().split(/\s+/).filter(Boolean);
    if (classes.length === 0)
        return "/* No classes */";
    const resolved = await (0, tailwindEngine_1.resolveClasses)(classes);
    const lines = [".element {"];
...
async function tailwindClassesToCssDeclarations(classString) {
    const classes = classString.trim().split(/\s+/).filter(Boolean);
    if (classes.length === 0)
        return "";
    const resolved = await (0, tailwindEngine_1.resolveClasses)(classes);
