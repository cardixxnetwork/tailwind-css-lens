 ...
 */
const CSS_RULE_TO_VARIANT: Record<string, string> = {};
for (const [variant, cssRule] of Object.entries(VARIANT_MAP)) {
 ...
 */
function cssRuleToVariantPrefix(rule: string): string {
  ...
  if (normalized.startsWith("&:")) {
    ...
    for (const [variant, css] of Object.entries(VARIANT_MAP)) {
      if (css === normalized) return variant + ":";
    }
 ...
 */
function parseVariantBlock(css: string): { wrappers: string[]; declarations: string } {
 ...
 */
const TAILWIND_PREFIX_TO_CSS_PROP: Record<string, string[]> = {
  flex: ["display"],
  grid: ["display"],
  block: ["display"],
  inline: ["display"],
...
function getConvertedCssProperties(tailwindClasses: string[]): Set<string> {
...
interface TaggedLine {
  /** The full CSS text without the tw tag (includes variant blocks if present) */
  cssText: string;
  /** Just the inner declarations (variant blocks stripped) */
  innerCss: string;
  /** Original Tailwind class if tagged, null if untagged */
  originalClass: string | null;
  /** Whether this is a continuation line (tw+ tag) */
  isContinuation: boolean;
  /** Variant wrappers extracted from CSS blocks */
  variantWrappers: string[];
}

/**
 * Parse a line from the CSS editor, extracting the tw tag and variant blocks.
 */
function parseTaggedLine(line: string): TaggedLine {
 ...
 */
function normalizeInnerCssForCompare(css: string): string {
 ...
 */
function tryUpdateOpacity(originalClass: string, currentCss: string, originalCss: string): string | null {
  ...
  const stripOpacity = (s: string) => s.replace(/\/\*\s*\d+%\s*opacity\s*\*\//, "").replace(/\s+/g, " ").replace(/;\s*$/, "").trim().toLowerCase();
 ...
 */
function convertNewCssToTailwind(cssText: string): string[] {
  const cleaned = stripCssComments(cssText).trim();
  if (!cleaned) return [];

  const normalizedCss = `.dummy { ${cleaned} }`.replace(
    /([^;\s}])\s*}/g,
    "$1; }"
  );

  const result = CssToTailwindTranslator(normalizedCss);
 ...
 */
function wrappersToVariantPrefix(wrappers: string[]): string {
...
export function convertCssToTailwind(cssInput: string): ConversionResult {
  ...
  const seenOriginalClasses = new Set<string>();
  ...
  while (i < lines.length) {
    ...
    if (parsed.originalClass && !parsed.isContinuation) {
      ...
      const cssLines: string[] = [parsed.innerCss];
      let j = i + 1;
